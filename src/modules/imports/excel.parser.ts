...
export function parseExcel(
  ...
  if (!sheetName)
    return { rows: [], errors: [{ row: 1, field: '文件', message: 'Excel中没有工作表' }] };
  ...
  if (!matrix.length)
    return { rows: [], errors: [{ row: 1, field: '文件', message: 'Excel内容为空' }] };
  ...
  for (let i = 1; i < matrix.length; i += 1) {
    ...
    if (name.length > 200)
      errors.push({ row, field: '项目名称', message: '项目名称不能超过200个字符' });
    if (annualGoal.length > 2000)
      errors.push({ row, field: '年度目标', message: '年度目标不能超过2000个字符' });
    if (ownerName && ownerName.length > 50)
      errors.push({ row, field: '负责人', message: '负责人不能超过50个字符' });
    if (progress === null) errors.push({ row, field: '当前进度', message: '进度必须在0到100之间' });
    if (name) {
    ...
    if (name && progress !== null) rows.push({ row, name, annualGoal, ownerName, progress });
  }
  if (rows.length > maxRows)
    errors.push({ row: 1, field: '文件', message: `有效数据行不能超过${maxRows}行` });
  if (!rows.length && !errors.length)
    errors.push({ row: 1, field: '文件', message: 'Excel中没有有效数据行' });
  return { rows, errors };
}
